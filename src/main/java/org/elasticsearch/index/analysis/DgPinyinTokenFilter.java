...
public class DgPinyinTokenFilter extends TokenFilter {
    ...
    protected int lastTermStartOffset = 0;
    ...
    public DgPinyinTokenFilter(TokenStream in, PinyinConfig config) {
        super(in);
        this.config = config;
        //validate config
    ...
    private boolean readTerm() {
        ...
        int isPositionIncrement = ((PackedTokenAttributeImpl) termAtt).getPositionIncrement();
        int originOrFullPositon = 0;
        if (isPositionIncrement > 0) {
            originOrFullPositon = 1;
        }
        if (config.keepOriginal && !processedOriginal) {
            processedOriginal = true;
            addCandidate(new TermItem(source, 0, source.length(), originOrFullPositon));
        }

        if (config.keepJoinedFullPinyin && !processedFullPinyinLetter && fullPinyinLetters.length() > 0) {
            processedFullPinyinLetter = true;
            addCandidate(new TermItem(fullPinyinLetters.toString(), 0, source.length(), originOrFullPositon));
        ...
        if (!processedSortCandidate) {
            ...
            Collections.sort(candidate, new Comparator<TermItem>() {
                @Override
    ...
    void setTerm(String term, int startOffset, int endOffset, int position) {
        ...
        if (lastTermStartOffset == offsetAttr.startOffset() && lastTermStartOffset > 0) {
            positionAttr.setPositionIncrement(0);
        } else {
            positionAttr.setPositionIncrement(offset);
            lastTermStartOffset = offsetAttr.startOffset();
        }
